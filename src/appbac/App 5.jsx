...
function WordManagement({ vocabulary, setVocabulary, currentListId, getInitialWords }) {
  ...
  const handleAdd = (e) => {
    ...
    const wordToAdd = {
      id: `user_${Date.now()}`,  // Prefix to distinguish user-added words
    ...
    userVocab[currentListId].words.push(wordToAdd)
    
    // Save updated user vocabulary
    saveUserVocabulary(userVocab)

    // Update the combined vocabulary in the app state
...
const importUserVocabulary = (event) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (isValidVocabularyStructure(importedVocab)) {
          const currentUserVocab = getUserVocabulary()
          // Merge imported vocabulary with current user vocabulary
          Object.keys(importedVocab).forEach(category => {
            currentUserVocab[category].words = [
              ...currentUserVocab[category].words,
              ...importedVocab[category].words
            ]
          })
          saveUserVocabulary(currentUserVocab)
          setVocabulary(getInitialWords(currentListId))
 ...
 const handleKeyPress = (e) => {
  ...
  : currentWord.exampleSentences[0].spanish.split('_____').map((part, index, array) => (
      <React.Fragment key={index}>  {/* Add this key */}
        {part}
